...
action_ctrl.move = function(self, monster, start_pos, end_pos, attack_target, skill_target_pos)
	local cb = function()
		...
		elseif skill_target_pos then
			self:use_skill_directly(monster, skill_target_pos)
		else
...
action_ctrl.use_skill_directly = function(self, monster, target_pos_num)
	monster:get_skill():play()
	local cost = monster:get_skill():get_cost()
	self:update_anger(monster)
	
	local cb = function()
		monster:get_skill():use(target_pos_num)
		local cb2 = function()
			self._final_call_back()
		end
		local callback2 = cc.CallFunc:create(cb2)
		gtool:do_something_later(callback2, 0.6)
	end
	local callback = cc.CallFunc:create(cb)
	self:toward_to_int_pos(monster, monster:get_cur_pos_num(), target_pos_num)
	monster:do_animation("skill", callback)
end
...
action_ctrl.update_anger = function(self, monster)
	local cb = function()
		monster.card.update(monster:get_cur_anger())
		monster.blood_bar.update_anger(monster:get_cur_anger())
	end
